...
func _physics_process(delta: float) -> void:
	if target_node != null:
		if target_node is BaseBuilding1 and position.distance_to(target_node.position) <= 135:
			deal_damage(target_node)
		if target_node is StandardUnitFactory1 or target_node is FastUnitFactory1 or target_node is HeavyUnitFactory1 or target_node is MineBuilding1:
